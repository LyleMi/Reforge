...
use crate::model::{
    Finding, FindingKind, FindingMetric, FlowLocation, FlowWitness, FlowWitnessStep, MetricId,
    RelatedLocation,
};
...
pub(super) fn evaluate_policies(
    ...
    for policy in policies {
        let (mut policy_findings, truncated) = evaluate_boundary(&policy, graph, max_hops);
...
fn evaluate_boundary(
    ...
    (
        findings_for_candidates(policy, graph, &candidates),
        truncated,
    )
}
...
fn finding_for_path(
    ...
    let metrics = vec![
        FindingMetric::measurement(MetricId::FlowModuleHops, module_hops, "module transitions"),
        FindingMetric::measurement(MetricId::FlowCallEdges, call_edges, "call edges"),
        FindingMetric::measurement(MetricId::FlowPathSteps, candidate.path.edges.len(), "steps"),
        FindingMetric::measurement(MetricId::FlowUnresolvedEdges, 0, "edges"),
        FindingMetric::measurement(
            MetricId::FlowPolicyConformingPaths,
            usize::from(conforming_path.is_some()),
            "paths",
        ),
        FindingMetric::measurement(MetricId::FlowPolicyBypassPaths, bypass_count, "paths"),
    ];
    let mut finding = Finding::from(
        FindingInput::new(
    ...
    finding.flow_witness = Some(witness);
    finding.refresh_id();
    finding
}
