...
describe("evidence view state",()=>{
  ...
  it("rejects unknown files",()=>{const report={schema_version:22,raw_metrics:{files:[{path:"src/main.rs"}]},findings:[],issues:[]} as never;expect(parseViewState("#map?file=missing.rs",report).file).toBeNull()});
});
