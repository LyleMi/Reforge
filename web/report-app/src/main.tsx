...
function UnityOverview({ report }: { report: ScanReport }) {
  const unity = report.unity_project;
  if (!unity || unity.status === "not_detected" || unity.status === "disabled") return null;
  const stats = unity.stats ?? {};
  return <Section title="Unity project" meta={`${formatKind(unity.status)} · ${unity.editor_version ?? "unknown Editor"}`}>
    <div className="unity-overview"><div className="summary-grid"><SummaryCard label="Assemblies" value={formatNumber(number(stats.assemblies))} meta={`${unity.assembly_edges?.length ?? 0} edges`}/><SummaryCard label="Scenes" value={formatNumber(number(stats.scenes))} meta={`${formatNumber(number(stats.prefabs))} prefabs`}/><SummaryCard label="Assets" value={formatNumber(number(stats.assets))} meta={`${formatNumber(number(stats.guids))} GUIDs`}/><SummaryCard label="Tests" value={formatNumber(number(stats.tests))} meta={unity.serialization_mode ?? "serialization unknown"}/></div>
    {unity.degraded_reasons?.length ? <div className="unity-degraded"><strong>Coverage limitations</strong>{unity.degraded_reasons.map(reason=><p key={reason}>{reason}</p>)}</div> : null}
    {unity.assemblies?.length ? <div className="unity-assemblies">{unity.assemblies.slice(0,12).map(assembly=><span key={`${assembly.name}-${assembly.path}`}><strong>{assembly.name}</strong>{assembly.editor_only ? " · Editor" : ""}{assembly.test_assembly ? " · tests" : ""}{assembly.predefined ? " · predefined" : ""}</span>)}</div> : null}</div>
  </Section>;
}
