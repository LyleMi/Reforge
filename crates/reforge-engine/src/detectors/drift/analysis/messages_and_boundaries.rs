...
pub(super) fn boundary_inventory(files: &[SourceFile], target: &Path) -> BoundaryInventory {
    let mut inventory = BoundaryInventory::default();

    for file in files
        .iter()
        .filter(|file| !is_test_source(&file.path))
        .filter(|file| same_package(&file.path, target))
    {
...
fn same_package(left: &Path, right: &Path) -> bool {
    package_prefix(left) == package_prefix(right)
}
...
fn package_prefix(path: &Path) -> Option<String> {
    ...
    for pair in components.windows(2) {
        if matches!(pair[0].as_ref(), "crates" | "tools") {
