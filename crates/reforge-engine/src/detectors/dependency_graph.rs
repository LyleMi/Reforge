...
use crate::model::{
    DependencyGraphEdge, DependencyGraphNode, DependencyGraphSnapshot, DetectedEvidence,
    DetectedMeasurement, MetricId, RelatedLocation, Rule,
};
...
#[cfg(test)]
pub(crate) fn scan_dependency_graph(sources: &[SourceFile], root: &Path) -> Vec<DetectedEvidence> {
    scan_dependency_graph_report(sources, root).detections
}
...
pub(crate) fn scan_dependency_graph_report(
    ...
    let mut detections = dependency_cycle_detections(&graph);
    detections.extend(dependency_hub_detections(&graph));
    DependencyGraphScan {
...
fn dependency_cycle_detections(graph: &DependencyGraph) -> Vec<DetectedEvidence> {
...
fn dependency_hub_detections(graph: &DependencyGraph) -> Vec<DetectedEvidence> {
    ...
    graph
        .nodes
        .values()
        .filter(|node| !is_rust_composition_root(&node.path))
        .filter_map(|node| dependency_hub_detection(graph, node, &context))
        .collect()
}

fn is_rust_composition_root(path: &str) -> bool {
    matches!(
...
fn dependency_hub_detection(
    graph: &DependencyGraph,
    node: &GraphNode,
    context: &HubContext,
) -> Option<DetectedEvidence> {
...
fn dependency_hub_metrics(
    ...
    if dependency_depth > 1 {
        metrics.push(DetectedMeasurement::threshold(
    ...
    metrics.push(DetectedMeasurement::measurement(
...
fn push_direct_coupling_metrics(
    metrics: &mut Vec<DetectedMeasurement>,
    fan_in: usize,
    fan_out: usize,
) {
    if fan_out > 0 {
