...
impl FunctionAnalyzer<'_> {
    ...
    fn process_assignment(&mut self, assignment: Node<'_>) {
        ...
        if left.kind() == "field_expression" {
            let targets = self.eval_field(left, FlowEdgeKind::FieldWrite);
            for target in targets {
                self.connect_sources_kind(
                    &sources,
                    target,
    ...
    fn eval_field(&mut self, field: Node<'_>, edge_kind: FlowEdgeKind) -> Vec<NodeId> {
        ...
        let name = field
            ...
            .and_then(|node| text(node, self.file))
        ...
        self.connect_sources_kind(
            ...
            edge_kind,
            line,
            format!("{} field {name}", if edge_kind == FlowEdgeKind::FieldRead { "read" } else { "write" }),
        );
        vec![node]
    }
    ...
    fn connect_sources(&mut self, sources: &[NodeId], target: NodeId, line: usize, name: String) {
        self.connect_sources_kind(sources, target, FlowEdgeKind::Assignment, line, name);
    }

    fn connect_sources_kind(
        &mut self,
        sources: &[NodeId],
        target: NodeId,
        kind: FlowEdgeKind,
