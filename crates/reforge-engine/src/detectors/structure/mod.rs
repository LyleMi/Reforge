...
pub(crate) fn scan_parsed_structure(
    ...
    signals.detections.extend(group_occurrences(
        signals.literals,
    ...
    signals
        .detections
        .extend(happy_path_test_detections(signals.happy_path_test_files));
    signals
        .detections
        .extend(file_naming_drift_detections(&signals.naming_directories));
    signals.detections.extend(directory_drift_detections(
        &signals.directory_files,
        options,
    ));

    Ok(signals.detections)
}
...
include!("function_detections.rs");
