fn scan_function_metrics(
    ...
    for function in functions {
        ...
        for signal in FUNCTION_DETECTION_SIGNALS {
            if signal.exceeds(function, options) {
                readability_signals.push(signal);
            }
            push_function_threshold_detection(file, function, options, signals, signal);
        }
        collect_data_clumps(file, function, options, signals);
    }
}

const FUNCTION_DETECTION_SIGNALS: [FunctionDetectionSignal; 4] = [
    FunctionDetectionSignal::LongFunction,
    FunctionDetectionSignal::ComplexFunction,
    FunctionDetectionSignal::DeepNesting,
    FunctionDetectionSignal::ManyParameters,
];

#[derive(Debug, Clone, Copy)]
enum FunctionDetectionSignal {
    LongFunction,
    ComplexFunction,
    DeepNesting,
    ManyParameters,
...
fn push_function_threshold_detection(
    file: &SourceFile,
    function: &FunctionMetric,
    options: &StructureOptions,
    signals: &mut FileSignals,
    signal: FunctionDetectionSignal,
) {
    if !signal.exceeds(function, options) {
        return;
    }
    ...
    signals
        .detections
        .push(crate::model::DetectedEvidence::from(DetectedEvidenceInput::new(
            signal.kind(),
...
impl FunctionDetectionSignal {
    ...
    fn kind(self) -> Rule {
        match self {
            Self::LongFunction => Rule::LongFunction,
            Self::ComplexFunction => Rule::ComplexFunction,
            Self::DeepNesting => Rule::DeepNesting,
            Self::ManyParameters => Rule::ManyParameters,
        }
    }
