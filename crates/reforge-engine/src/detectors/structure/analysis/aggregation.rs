...
pub(in crate::detectors::structure) fn file_naming_drift_detections(
...
pub(in crate::detectors::structure) fn directory_drift_detections(
    ...
    for (directory, concepts) in directories {
        ...
        if concepts.len() > threshold {
            detections.push(crate::model::DetectedEvidence::from(DetectedEvidenceInput::new(
                Rule::DirectoryDrift,
...
pub(in crate::detectors::structure) fn group_occurrences(
    ...
    for (key, mut group) in by_key {
        ...
        let detection = crate::model::DetectedEvidence::from(
            DetectedEvidenceInput::new(
                kind,
