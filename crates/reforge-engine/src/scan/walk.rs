fn resolve_scan_root(args: &EffectiveConfig) -> Result<PathBuf> {
    args.path
        .canonicalize()
...
fn scan_sources(
    ...
    for (index, path) in source_plan.source_files.iter().enumerate() {
        let file_options = FileScanOptions {
            ...
            codebase: options.execution.codebase,
        };
        scan_file(path, file_options, scan)?;
        report_file_scan_progress(progress, index + 1, options.total_source_files, path);
    }

    if options.execution.codebase {
        scan_directories(
...
fn scan_file(
    ...
    record_file_observations(&display_path, &source, is_test, options, scan);
...
fn record_file_observations(
    ...
    if options.codebase && line_count > options.max_file_lines {
        scan.detections.push(DetectedEvidence::from(DetectedEvidenceInput::new(
            Rule::LargeFile,
            display_path,
            Some(1),
...
fn scan_structure_source(
    ...
    if is_supported_structure_source(path) {
        ...
        scan.codebase_sources.push(source_file);
    }
    Ok(())
}
...
fn scan_directories(
    ...
    for (directory, file_count) in directory_source_files {
        ...
        if *file_count > max_dir_files {
            detections.push(DetectedEvidence::from(DetectedEvidenceInput::new(
                Rule::LargeDirectory,
...
fn should_visit_entry(entry: &DirEntry, root: &Path, args: &EffectiveConfig) -> bool {
...
fn is_ignored_path(path: &Path, root: &Path, args: &EffectiveConfig) -> bool {
...
fn is_excluded_test_path(path: &Path, args: &EffectiveConfig) -> bool {
    args.filters.exclude_tests && (is_test_source(path) || csharp_source_declares_tests(path))
}

fn should_scan_source_file(path: &Path, args: &EffectiveConfig) -> bool {
